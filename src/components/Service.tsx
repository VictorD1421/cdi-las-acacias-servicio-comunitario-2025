...
import { Smile, Users, HeartHandshake, Gift } from "lucide-react";
...
export default function Service({ data }: ServiceProps) {
  // compute lg-cols based on number of options
  const cols =
    ...
    <section className="relative bg-sky-50 pt-16 pb-20">
      ...
      <div className="container mx-auto px-4 -mt-8">
        <div className="bg-white rounded-xl shadow-lg border-t-4 border-sky-200 p-8 text-center">
          <h2
            className={`${heebo.className} text-3xl md:text-4xl lg:text-5xl font-bold text-sky-600 mb-2`}
          >
            {data.title}
          </h2>
      ...
      <div className="container mx-auto px-4 mt-12">
        <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 ${cols} gap-8`}>
              ...
              <div className="group bg-white rounded-lg border border-gray-200 p-6 text-center transition-shadow shadow-sm hover:shadow-xl">
                <div className="mx-auto mb-4 flex h-20 w-20 items-center justify-center rounded-full border-2 border-gray-200 bg-white transition-colors group-hover:border-thrustwhorthy-brown">
                  {React.createElement(item.icon, {
                    className: "h-10 w-10 text-sky-500 group-hover:text-thrustwhorthy-brown",
                  })}
                </div>
                <h3
                  className={`${heebo.className} text-lg font-semibold text-sky-600 group-hover:text-xl transition-all duration-200 mb-2`}
                >
                  {item.title}
                </h3>
